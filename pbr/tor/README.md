...
> Drafted with LLM assistance and verified against the pbr source, the dnsmasq
> and Tor manuals, and tests on a live OpenWrt router. Paths, defaults and
> behaviour change between releases and the ones quoted here come from one
> system at one point in time — check the commands against your own.

This is the deep-dive companion to the Tor section of the
[pbr documentation](https://docs.mossdef.org/pbr/1.2.3/#tor), which covers what most people need
for a working setup. Everything below is the mechanism behind it, and how to
diagnose it when it does not work.
...
| **If it's down**    | kill-switch / leak per policy               | connection refused on those ports |
...
> **If you just want Tor to work reliably, use mode A: set `src_addr` and leave
> `dest_addr` empty.** It matches on the first packet, needs no DNS cooperation,
> and none of the ten failure modes catalogued further down can touch it. Mode B
> — matching on a destination domain — is supported and useful for ordinary
> destinations, but it depends on `dnsmasq` behaviour that pbr cannot see or
> verify, and on stock OpenWrt it does not work for `.onion` at all until you
> deal with `rfc6761.conf`.
>
> The one tradeoff: mode A sends **all** of that client's DNS through Tor, so
> local filtering (adblock and friends) stops applying to it.
...
A working set looks like this — the addresses are Tor's virtual ones, inside
whatever `VirtualAddrNetworkIPv4` specifies:

```sh
nft list set inet fw4 pbr_tor_4_dst_ip_cfg0c6ff5
```
...
If instead you see public addresses (40.x, 140.x, 185.x …), something other
than Tor answered the `.onion` lookup and pbr stored what DNS returned. If
there is no `elements` line at all, nothing has been resolved yet — or the
lookups are not reaching dnsmasq.
