...
| force_ip_family    | auto/ipv4/ipv6       | auto      | IP family used to reach DNS resolvers. `auto`=dual-stack, no `-4`; `ipv4`=IPv4 only, adds `-4`; `ipv6`=IPv6 only. If the forced family has no matching bootstrap, Cloudflare/Google servers of that family are added.                 |
